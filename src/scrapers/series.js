...
const seriesScraper = async (page = 1) => {
    const requestUrl = `${url}/series/page/${page}/`
    let data
    try {
        ({ data } = await axios.get(requestUrl, { headers: Header, timeout: 10000 }))
    } catch (err) {
        throw upstreamError("series scraper", requestUrl, err)
    }
    const $ = await cheerio.load(data)
    const anime = []
    $("#aa-movies li.series").each((_, el) => {
        const title = $(el).find("h2.entry-title").text().trim()
        const anime_id = extractPathSlug($(el).find("a.lnk-blk").attr("href"), "series")
